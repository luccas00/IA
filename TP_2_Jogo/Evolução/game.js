...
// --- IA TP2: busca gulosa, A* e navegação por nível ---
// A arena do jogo usa coordenadas contínuas (x/y). Para aplicar busca,
// criamos uma grade lógica sobre a arena e tratamos cada célula livre como um nó.
const IA_GRID_SIZE = 50;
const IA_GRID_COLS = Math.ceil(arena.width / IA_GRID_SIZE);
...
function worldToGrid(x, y) {
    return {
        r: clamp(Math.floor(y / IA_GRID_SIZE), 0, IA_GRID_ROWS - 1),
        c: clamp(Math.floor(x / IA_GRID_SIZE), 0, IA_GRID_COLS - 1)
    };
}

function gridToWorld(cell) {
    return {
        x: clamp(cell.c * IA_GRID_SIZE + IA_GRID_SIZE / 2, 0, arena.width),
        y: clamp(cell.r * IA_GRID_SIZE + IA_GRID_SIZE / 2, 0, arena.height)
    };
}
...
function manhattanGrid(a, b) {
    return Math.abs(a.r - b.r) + Math.abs(a.c - b.c);
}

function isGridBlocked(cell, radius = 15) {
    if (!cell || cell.r < 0 || cell.c < 0 || cell.r >= IA_GRID_ROWS || cell.c >= IA_GRID_COLS) return true;
    let p = gridToWorld(cell);
    if (p.x < radius || p.x > arena.width - radius || p.y < radius || p.y > arena.height - radius) return true;
    return walls.some(w => checkCollision({ x: p.x, y: p.y, radius }, w));
}

function getGridNeighbors(cell, radius = 15) {
    let candidates = [
        { r: cell.r - 1, c: cell.c },
        { r: cell.r + 1, c: cell.c },
        { r: cell.r, c: cell.c - 1 },
    ...
    return candidates.filter(c => !isGridBlocked(c, radius));
}

function nearestWalkableGrid(cell, radius = 15) {
    if (!isGridBlocked(cell, radius)) return cell;
    let best = null;
    let bestDist = Infinity;
    for (let r = 0; r < IA_GRID_ROWS; r++) {
        for (let c = 0; c < IA_GRID_COLS; c++) {
            let candidate = { r, c };
            if (isGridBlocked(candidate, radius)) continue;
            let d = manhattanGrid(cell, candidate);
            if (d < bestDist) {
                bestDist = d;
                best = candidate;
            }
...
function reconstructGridPath(parent, goalKey) {
    let path = [];
...
function aStarGrid(startWorld, goalWorld, radius = 15) {
    let start = nearestWalkableGrid(worldToGrid(startWorld.x, startWorld.y), radius);
    let goal = nearestWalkableGrid(worldToGrid(goalWorld.x, goalWorld.y), radius);
    let startKey = gridKey(start);
    let goalKey = gridKey(goal);
    let frontier = [{ cell: start, g: 0, f: manhattanGrid(start, goal) }];
    let parent = { [startKey]: null };
    let bestCost = { [startKey]: 0 };
    let closed = new Set();

    while (frontier.length > 0) {
        frontier.sort((a, b) => a.f - b.f);
        let current = frontier.shift();
        let currentKey = gridKey(current.cell);
        if (closed.has(currentKey)) continue;
        if (currentKey === goalKey) return reconstructGridPath(parent, goalKey);
        closed.add(currentKey);

        for (let neighbor of getGridNeighbors(current.cell, radius)) {
            let nk = gridKey(neighbor);
            let newCost = current.g + 1;
            if (!(nk in bestCost) || newCost < bestCost[nk]) {
                bestCost[nk] = newCost;
                parent[nk] = currentKey;
                frontier.push({ cell: neighbor, g: newCost, f: newCost + manhattanGrid(neighbor, goal) });
            }
        }
    }
    return [];
...
function garantirMemoriaEquipe(teamId) {
    if (!iaSharedTeamMemory[teamId]) {
        iaSharedTeamMemory[teamId] = {
            blocked: new Set(),
            known: new Set(),
            enemy: null,
            caixas: new Map()
    ...
    return iaSharedTeamMemory[teamId];
}

function garantirMemoriaOnlineBot(bot) {
    if (!bot.aiKnownCells) bot.aiKnownCells = new Set();
    if (!bot.aiBlockedCells) bot.aiBlockedCells = new Set();
    return bot;
}

function perceberAmbienteOnline(bot) {
    ...
    for (let dr = -raioPercepcao; dr <= raioPercepcao; dr++) {
        for (let dc = -raioPercepcao; dc <= raioPercepcao; dc++) {
            ...
            if (!bot.aiKnownCells.has(key)) reveladas++;

            bot.aiKnownCells.add(key);
            memoriaEquipe.known.add(key);

            if (isGridBlocked(cell, (bot.radius || 15) + IA_NAV_MARGIN)) {
                bot.aiBlockedCells.add(key);
                memoriaEquipe.blocked.add(key);
            }
        }
    }
...
function getGridNeighborsOnline(cell, bot, radius = 15) {
    ...
    return candidates.filter(c => {
        if (!c || c.r < 0 || c.c < 0 || c.r >= IA_GRID_ROWS || c.c >= IA_GRID_COLS) return false;

        let key = gridKey(c);

        // Nível 4/5: o agente não usa o mapa real completo diretamente.
        // Ele evita apenas bloqueios que já foram percebidos ou compartilhados.
        if (bot.aiBlockedCells.has(key) || memoriaEquipe.blocked.has(key)) return false;

        let p = gridToWorld(c);
        if (p.x < radius || p.x > arena.width - radius || p.y < radius || p.y > arena.height - radius) return false;

        return true;
    });
}

function aStarGridOnline(startWorld, goalWorld, bot, radius = 15) {
    let start = worldToGrid(startWorld.x, startWorld.y);
    let goal = worldToGrid(goalWorld.x, goalWorld.y);
    let startKey = gridKey(start);
    let goalKey = gridKey(goal);
    ...
    while (frontier.length > 0) {
        ...
        for (let neighbor of getGridNeighborsOnline(current.cell, bot, radius)) {
            let nk = gridKey(neighbor);
            let memoriaEquipe = garantirMemoriaEquipe(bot.teamId);

            // Células desconhecidas são permitidas, mas recebem pequena penalização.
            // Isso mantém a busca online exploratória sem ignorar o mapa já conhecido.
            let unknownPenalty = (!bot.aiKnownCells || !bot.aiKnownCells.has(nk)) && !memoriaEquipe.known.has(nk) ? 0.25 : 0;
            let newCost = current.g + 1 + unknownPenalty;
...
function moverBotComAStarOnline(bot, alvo, now) {
    if (!alvo) return false;

    perceberAmbienteOnline(bot);

    let targetKey = `${Math.round(alvo.x / 25)},${Math.round(alvo.y / 25)}`;
    let precisaRecalcular = !bot.aiPath || bot.aiTargetKey !== targetKey || now - (bot.aiLastPlan || 0) > 260;

    if (precisaRecalcular) {
        bot.aiPath = aStarGridOnline({ x: bot.x, y: bot.y }, alvo, bot, (bot.radius || 15) + IA_NAV_MARGIN);
        bot.aiTargetKey = targetKey;
        bot.aiLastPlan = now;
        bot.aiPathIndex = 1;
        bot.aiReplans = (bot.aiReplans || 0) + 1;
    }

    if (!bot.aiPath || bot.aiPath.length < 2) return false;

    let idx = clamp(bot.aiPathIndex || 1, 1, bot.aiPath.length - 1);
    let cell = bot.aiPath[idx];

    // Ao descobrir que a próxima célula é parede real, marca como conhecida e replaneja.
    if (isGridBlocked(cell, (bot.radius || 15) + IA_NAV_MARGIN)) {
        let key = gridKey(cell);
        garantirMemoriaOnlineBot(bot);
        bot.aiBlockedCells.add(key);
        garantirMemoriaEquipe(bot.teamId).blocked.add(key);
        bot.aiPath = null;
        bot.aiLastPlan = 0;
        return false;
    }

    let waypoint = gridToWorld(cell);

    if (Math.hypot(waypoint.x - bot.x, waypoint.y - bot.y) < 18 && idx < bot.aiPath.length - 1) {
        bot.aiPathIndex = idx + 1;
        waypoint = gridToWorld(bot.aiPath[bot.aiPathIndex]);
    }

    let ang = Math.atan2(waypoint.y - bot.y, waypoint.x - bot.x);
    let moveu = moverBotSeguro(bot, ang, bot.speed);

    if (!moveu) {
        let atual = worldToGrid(bot.x, bot.y);
        bot.aiBlockedCells.add(gridKey(atual));
        garantirMemoriaEquipe(bot.teamId).blocked.add(gridKey(atual));
        bot.aiPath = null;
        bot.aiLastPlan = 0;
    }
...
function atualizarMemoriaDeCombate(bot, alvoM, visaoLimpa, now) {
    let memoriaEquipe = garantirMemoriaEquipe(bot.teamId);
    if (alvoM && visaoLimpa) {
        memoriaEquipe.enemy = {
            x: alvoM.x,
        ...
        bot.aiLastKnownEnemy = memoriaEquipe.enemy;
    } else if (memoriaEquipe.enemy && now - memoriaEquipe.enemy.seenAt <= IA_SHARED_MEMORY_MAX_AGE) {
        bot.aiLastKnownEnemy = memoriaEquipe.enemy;
    }
}

function getPontoCobertura(bot, inimigo) {
    ...
    walls.forEach(w => {
        let pontos = [
            { x: w.x - 28, y: w.y + w.height / 2 },
            { x: w.x + w.width + 28, y: w.y + w.height / 2 },
            { x: w.x + w.width / 2, y: w.y - 28 },
            { x: w.x + w.width / 2, y: w.y + w.height + 28 }
        ];

        pontos.forEach(p => {
            if (p.x < 25 || p.x > arena.width - 25 || p.y < 25 || p.y > arena.height - 25) return;
            if (isGridBlocked(worldToGrid(p.x, p.y), (bot.radius || 15) + IA_NAV_MARGIN)) return;

            // Precisa haver uma parede entre a cobertura e o inimigo.
            if (!temParede(p.x, p.y, inimigo.x, inimigo.y)) return;

            let distBot = Math.hypot(p.x - bot.x, p.y - bot.y);
            let distEnemy = Math.hypot(p.x - inimigo.x, p.y - inimigo.y);

            if (distEnemy < 140 || distEnemy > 520) return;

            candidatos.push({ p, score: distBot + Math.abs(distEnemy - 280) });
        });
    });
...
function getPontoFlanco(bot, inimigo) {
    if (!inimigo) return null;

    let indice = 0;
    let m = String(bot.id || "").match(/\d+/);
    if (m) indice = parseInt(m[0], 10);

    let angBase = Math.atan2(bot.y - inimigo.y, bot.x - inimigo.x);
    let offset = ((indice % 3) - 1) * 0.85;
    let raio = 210 + (indice % 2) * 55;

    let ponto = {
        x: clamp(inimigo.x + Math.cos(angBase + offset) * raio, 40, arena.width - 40),
        y: clamp(inimigo.y + Math.sin(angBase + offset) * raio, 40, arena.height - 40)
    };

    if (isGridBlocked(worldToGrid(ponto.x, ponto.y), (bot.radius || 15) + IA_NAV_MARGIN)) {
        ponto = getValidSpawn(false, false, bot.radius || 20);
    }

    return ponto;
}

function selecionarObjetivoCooperativo(bot, alvoM, menorD, visaoLimpa, now) {
    atualizarMemoriaDeCombate(bot, alvoM, visaoLimpa, now);

    let memoriaEquipe = garantirMemoriaEquipe(bot.teamId);
    let inimigoConhecido = alvoM;

    if (!inimigoConhecido && memoriaEquipe.enemy && now - memoriaEquipe.enemy.seenAt <= IA_SHARED_MEMORY_MAX_AGE) {
        inimigoConhecido = memoriaEquipe.enemy;
    }

    // Se estiver vulnerável, prioriza vida ou cobertura.
    if (bot.hp < 45) {
        let caixaVida = caixas
            .filter(cx => cx.tipo === "vida")
            .sort((a, b) => Math.hypot(a.x - bot.x, a.y - bot.y) - Math.hypot(b.x - bot.x, b.y - bot.y))[0];

        if (caixaVida) {
            return { alvo: caixaVida, tipo: "caixa_vida", distancia: Math.hypot(caixaVida.x - bot.x, caixaVida.y - bot.y) };
        }

        let cobertura = getPontoCobertura(bot, inimigoConhecido);
        if (cobertura) {
            return { alvo: cobertura, tipo: "cobertura", distancia: Math.hypot(cobertura.x - bot.x, cobertura.y - bot.y) };
        }
    }

    // Se enxerga ou recebeu informação do inimigo, tenta flanquear/coordenar.
    if (inimigoConhecido) {
        let cobertura = getPontoCobertura(bot, inimigoConhecido);
        if (cobertura && visaoLimpa && menorD < 230) {
            return { alvo: cobertura, tipo: "cobertura", distancia: Math.hypot(cobertura.x - bot.x, cobertura.y - bot.y) };
        }

        let flanco = getPontoFlanco(bot, inimigoConhecido);
...
function getInimigosDoBot(bot) {
    let alvos = [];
    if (player && player.hp > 0 && player.teamId !== bot.teamId) alvos.push(player);
    bots.forEach(o => {
        if (o !== bot && o.hp > 0 && o.teamId !== bot.teamId) alvos.push(o);
    });
    return alvos;
}

function getInimigoMaisProximo(bot) {
    let melhor = null;
    let menor = Infinity;
    getInimigosDoBot(bot).forEach(o => {
        let d = Math.hypot(o.x - bot.x, o.y - bot.y);
        if (d < menor) {
            menor = d;
            melhor = o;
    ...
    return { alvo: melhor, distancia: menor };
}

function getCaixaMaisRelevante(bot, limite = Infinity) {
    let melhor = null;
    ...
    caixas.forEach(cx => {
        ...
        if (d > limite) return;
        let prioridade = 0;
        if (cx.tipo === "vida" && bot.hp < 70) prioridade = -180;
        else if (cx.tipo === "escudo" && bot.armor < 30) prioridade = -120;
        else if (cx.tipo === "arma" && bot.damage < 20) prioridade = -90;
        else if (cx.tipo === "velocidade") prioridade = -35;
        let score = d + prioridade;
        if (score < menorScore) {
            menorScore = score;
            melhor = cx;
        }
...
function getPontoPatrulha(bot) {
    if (!bot.patrolPoint || Math.hypot(bot.patrolPoint.x - bot.x, bot.patrolPoint.y - bot.y) < 35) {
        bot.patrolPoint = getValidSpawn(false, false, bot.isBoss ? 60 : 20);
    }
    return bot.patrolPoint;
}

function moverBotSeguro(bot, ang, velocidade = bot.speed) {
    bot.angle = ang;

    // Primeiro tenta o movimento principal.
    let antesX = bot.x;
    let antesY = bot.y;
    let resultado = moverEntidade(bot, Math.cos(ang) * velocidade, Math.sin(ang) * velocidade);
    if (resultado && resultado.moved) return true;

    // Se bateu em parede/canto, tenta escapar pela tangente.
    // Isso evita o efeito de o bot ficar "raspando" na parede indefinidamente.
    let tentativas = [Math.PI / 2, -Math.PI / 2, Math.PI / 4, -Math.PI / 4, Math.PI];
    for (let offset of tentativas) {
        let teste = ang + offset;
        resultado = moverEntidade(bot, Math.cos(teste) * velocidade, Math.sin(teste) * velocidade);
        if (resultado && resultado.moved) {
            bot.angle = teste;
            return true;
        }
    }

    // Caso extremo: se nada funcionou, invalida o caminho planejado.
    bot.aiPath = null;
    return Math.hypot(bot.x - antesX, bot.y - antesY) > 0.1;
}

function moverBotDireto(bot, alvo, multiplicador = 1) {
    if (!alvo) return;
    let ang = Math.atan2(alvo.y - bot.y, alvo.x - bot.x);
    moverBotSeguro(bot, ang, bot.speed * multiplicador);
}

function moverBotComDesvioSimples(bot, alvo) {
    if (!alvo) return;
    let ang = Math.atan2(alvo.y - bot.y, alvo.x - bot.x);
    bot.angle = ang;
    if (!temParede(bot.x, bot.y, alvo.x, alvo.y)) {
        moverBotSeguro(bot, ang, bot.speed);
        return;
    }

    // Nível 2: se a reta está bloqueada, tenta pequenas variações de ângulo.
    let tentativas = [Math.PI / 4, -Math.PI / 4, Math.PI / 2, -Math.PI / 2, Math.PI];
    for (let offset of tentativas) {
        let teste = ang + offset;
        let tx = bot.x + Math.cos(teste) * IA_GRID_SIZE;
        let ty = bot.y + Math.sin(teste) * IA_GRID_SIZE;
        if (!temParede(bot.x, bot.y, tx, ty)) {
            bot.angle = teste;
            moverBotSeguro(bot, teste, bot.speed);
            return;
        }
    }
}

function moverBotComAStar(bot, alvo, now) {
    if (!alvo) return false;
    let targetKey = `${Math.round(alvo.x / 25)},${Math.round(alvo.y / 25)}`;
    let precisaRecalcular = !bot.aiPath || bot.aiTargetKey !== targetKey || now - (bot.aiLastPlan || 0) > 350;

    if (precisaRecalcular) {
        bot.aiPath = aStarGrid({ x: bot.x, y: bot.y }, alvo, (bot.radius || 15) + IA_NAV_MARGIN);
        bot.aiTargetKey = targetKey;
        bot.aiLastPlan = now;
        bot.aiPathIndex = 1;
    }
    ...
    if (!bot.aiPath || bot.aiPath.length < 2) return false;
    let idx = clamp(bot.aiPathIndex || 1, 1, bot.aiPath.length - 1);
    let waypoint = gridToWorld(bot.aiPath[idx]);
    ...
    // Se não conseguiu avançar, força novo planejamento no próximo frame.
    if (!moveu) {
        bot.aiPath = null;
        bot.aiLastPlan = 0;
    }
...
function selecionarObjetivoIA(bot, alvoM, menorD, visaoLimpa = false, now = Date.now()) {
    ...
    return { alvo: patrulha, tipo: "patrulha", distancia: Math.hypot(patrulha.x - bot.x, patrulha.y - bot.y) };
}
...
function aplicarIABotPorNivel(bot, alvoM, menorD, visaoLimpa, now) {
    ...
    // Nível 1: busca gulosa. Decide pelo objetivo mais relevante/próximo e move em linha reta.
    if (dificuldade <= 1) {
        moverBotDireto(bot, alvo);
        return;
    }

    // Nível 2: mantém a lógica gulosa, mas tenta desviar quando há parede na rota direta.
    if (dificuldade === 2) {
        moverBotComDesvioSimples(bot, alvo);
        return;
    }
    ...
    // Nível 4: busca online com mapa interno parcial e compartilhamento de informações.
    if (dificuldade === 4) {
        atualizarMemoriaDeCombate(bot, alvoM, visaoLimpa, now);
    ...
    // Nível 5: usa busca online + memória compartilhada + flanco/cobertura.
    if (dificuldade >= 5) {
        if ((decisao.tipo === "flanco" || decisao.tipo === "cobertura") && Math.hypot(alvo.x - bot.x, alvo.y - bot.y) < 45) {
            let memoriaEquipe = garantirMemoriaEquipe(bot.teamId);
            if (memoriaEquipe.enemy && now - memoriaEquipe.enemy.seenAt <= IA_SHARED_MEMORY_MAX_AGE) {
                let ang = Math.atan2(memoriaEquipe.enemy.y - bot.y, memoriaEquipe.enemy.x - bot.x);
                bot.angle = ang;
            }
            return;
        }
        ...
        let moveu = moverBotComAStarOnline(bot, alvo, now);
        if (!moveu) moverBotComAStar(bot, alvo, now) || moverBotComDesvioSimples(bot, alvo);
    }
}

function resolverBotPresoEmParede(bot) {
    // Correção defensiva: se algum spawn/movimento deixar o bot encostado demais
    // em uma parede, empurra levemente para fora antes do próximo planejamento.
    for (let w of walls) {
        if (!checkCollision({ x: bot.x, y: bot.y, radius: bot.radius }, w)) continue;

        let closestX = Math.max(w.x, Math.min(bot.x, w.x + w.width));
        let closestY = Math.max(w.y, Math.min(bot.y, w.y + w.height));
        let dx = bot.x - closestX;
        let dy = bot.y - closestY;
        let dist = Math.hypot(dx, dy) || 0.001;
        let overlap = bot.radius - dist + 3;

        if (overlap > 0) {
            bot.x += (dx / dist) * overlap;
            bot.y += (dy / dist) * overlap;
            bot.x = clamp(bot.x, bot.radius, arena.width - bot.radius);
            bot.y = clamp(bot.y, bot.radius, arena.height - bot.radius);
            bot.aiPath = null;
        }
    }
}

function resolverSeparacaoBots() {
    // Evita dois bots ocupando o mesmo ponto e bloqueando um ao outro.
    // Não altera HP, tiro, times ou regras de dano; apenas separa fisicamente os corpos.
    for (let i = 0; i < bots.length; i++) {
        ...
        for (let j = i + 1; j < bots.length; j++) {
            ...
            let minDist = Math.max(IA_MIN_BOT_DISTANCE, (a.radius || 15) + (b.radius || 15) + 4);
            let dx = b.x - a.x;
            let dy = b.y - a.y;
            ...
            if (dist > 0 && dist < minDist) {
                ...
                moverEntidade(a, -nx * push, -ny * push);
                moverEntidade(b, nx * push, ny * push);

                a.aiPath = null;
                b.aiPath = null;
            } else if (dist === 0) {
                // Caso extremo: dois bots exatamente no mesmo pixel.
                let ang = ((i + j) * 0.73) % (Math.PI * 2);
                moverEntidade(a, -Math.cos(ang) * 2, -Math.sin(ang) * 2);
                moverEntidade(b, Math.cos(ang) * 2, Math.sin(ang) * 2);
                a.aiPath = null;
                b.aiPath = null;
            }
        }

        resolverBotPresoEmParede(a);
    }
}
...
function moverEntidade(entidade, movX, movY) {
    let oldX = entidade.x;
    let oldY = entidade.y;
    let nX = entidade.x + movX;
    let nY = entidade.y + movY;
    let canX = true;
    let canY = true;
    ...
    return {
        movedX: canX && Math.abs(entidade.x - oldX) > 0.01,
        movedY: canY && Math.abs(entidade.y - oldY) > 0.01,
        moved: Math.hypot(entidade.x - oldX, entidade.y - oldY) > 0.01
    };
}
...
function update() {
    ...
    for (let i = bots.length - 1; i >= 0; i--) {
        ...
        if (bot.isBoss) {
            ...
            aplicarIABotPorNivel(bot, alvoM, menorD, visaoLimpa, now);
            if (alvoM && visaoLimpa && now - bot.lastShot > bot.shootCooldown && menorD < 400) { let angleShoot = Math.atan2(alvoM.y - bot.y, alvoM.x - bot.x); bullets.push({ ownerId: bot.id, teamId: bot.teamId, x: bot.x + Math.cos(angleShoot) * 20, y: bot.y + Math.sin(angleShoot) * 20, angle: angleShoot, speed: 7, damage: bot.damage, skin: bot.skin }); bot.lastShot = now; tocarSom("tiroB"); }
        }
        if (Math.hypot(bot.x - oldX, bot.y - oldY) < bot.speed * 0.2) {
            bot.stuckCounter = (bot.stuckCounter || 0) + 1;
            if (bot.stuckCounter > 10) {
                // Destrava mais cedo: limpa rota A*, ignora alvo por pouco tempo e tenta sair pela lateral.
                bot.aiPath = null;
                bot.aiLastPlan = 0;
                bot.patrolPoint = getValidSpawn(false, false, bot.isBoss ? 60 : 20);
                bot.ignoreTargetUntil = now + 900;
                moverBotSeguro(bot, bot.angle + Math.PI / 2, bot.speed * 1.4);
                bot.stuckCounter = 0;
            }
        } else {
            bot.stuckCounter = 0;
